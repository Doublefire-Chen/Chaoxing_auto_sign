...
"""
#修改说明
##时间：2021年10月29日15:49:05
##作者：Doublefire.Chen
##删除了企业微信推送功能（考虑到大多数人不用企业推送）
##添加了sever酱推送功能
##对代码进行优化（新增了一个云函数的主函数），使其能部署到腾讯云函数上
##修改了签到完成退出方式，以适应腾讯云函数退出方式
##修改了一些注释
...
setting={
    ...
    "other":{
        ...
        "sleep":60 #每次检测间隔时间（S）默认60秒 一分钟，禁止调得过小，以免刷新速度过快导致系统将你的ID封锁，即不让你刷新，到时候程序就失效了。（别问我是怎么知道的，哈哈哈，当时等了一天多才解封）
    }
}
...
def getcourse():
    ...
    if('请重新登录' in res.text):
        print("Cookie已过期，请删除Cookie.txt和uid.txt文件并重启软件")
    else:
        d=json.loads(res.text)
